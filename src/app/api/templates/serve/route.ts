...
import { initDatabase, getUnseenTemplate, saveTemplate } from "../../../../lib/db";
import {
  stripMarkdown,
...
/**
 * Generates HTML content synchronously
...
/**
 * GET handler for serving templates
 * Checks cookies for seen templates, serves unseen template or generates new one
 * Triggers background generation for next time
 * Sets cookie with seen template ID
 * @param {NextRequest} request - The incoming request
 * @returns {Promise<NextResponse>} Template HTML and ID
 */
export async function GET(request: NextRequest) {
  try {
    ...
    if (template) {
      ...
      // Database is empty or all templates seen - generate synchronously
      html = await generateHTML();
      templateId = await saveTemplate(html);
    }

    // Update seen IDs
    const updatedSeenIds = [...seenIds, templateId];
    ...
    // Set cookie with updated seen IDs (10 years expiration)
    response.cookies.set("seen_templates", JSON.stringify(updatedSeenIds), {
